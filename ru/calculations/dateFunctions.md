...
## toLocalISO
Преобразует объект `Date` в строку в формате ISO, используя **локальное время** без применения временных сдвигов, связанных с текущей тайм-зоной браузера.  
Результат соответствует формату `YYYY-MM-DDTHH:mm:ss`.

### Синтаксис
```javascript
dateValue.toLocalISO();
```
### Параметры
нет
### Возвращаемое значение
string — строка, представляющая локальное время в ISO-формате без информации о временной зоне.
### Пример
```javascript
(new Date('2025-11-26T21:02:15')).toLocalISO();
```
