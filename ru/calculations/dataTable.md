...
Класс DataTable предназначен для работы с табличными данными. Он обеспечивает хранение 
и обработку структурированных данных, 
поддерживая операции фильтрации, сортировки, группировки и агрегации. Полный список доступных операций приведен в таблице [Методы](#методы).

## Свойства
- columns: [DataTableColumn](dataTableColumn.md)[] - массив объектов, описывающих колонки таблицы.
- rows: object[] - массив строк, где каждая строка представляет набор значений, соответствующих колонкам таблицы.

## Использование объекта DataTable
Обработка данных осуществляется через [методы](#методы) объекта DataTable. Многие методы DataTable поддерживают конвейерный способ вызова, что позволяет вызывать несколько методов подряд. Это делает код более лаконичным и читаемым, а также упрощает сложную обработку данных.
...
## createTable
Создает таблицу данных. Предусмотрено три варианта синтаксиса. Описание структуры колонок может быть передано в виде
массива объектов, массива строковых описаний или единого строкового описания.
...
```
### Параметры
- *columnSettings (необязательный)*: [DataTableColumn](dataTableColumn.md)[] - массив колонок таблицы. 
  Если массив пустой, будет создана пустая таблица.

или
- *columnSettings (необязательный)*: string[] - массив строковых описаний таблицы формата 'name \<dataType\>'. 
  Если массив пустой, будет создана пустая таблица. Установка значений по-умолчанию не предусмотрена.

или
- *columnSettings (необязательный)*: string - строковое описание колонок таблицы разделенное запятыми: 
- 'name1 \<dataType1\>, ..., nameN \<dataTypeN\>'.
...
```
Более компактный вариант синтаксиса. В качестве параметров передается массив строковых описаний колонок. 
Описание колонки должно соответствовать формату: 'name \<dataType\>'. Обращаем внимание, что установка значения по-умолчанию в данном варианте синтаксиса не предусмотрена.
```javascript
createTable(['product', 'qauntity number', 'amount number', 'is_available boolean'])
```
Компактный вариант синтаксиса. В качестве параметра передается строковое описание колонок таблицы. 
Описания отдельных колонок разделены запятыми.
```javascript
createTable('product, qauntity number, amount number, is_available boolean')
```
...
или 
- column: string - строковое описание колонки таблицы формата 'name \<dataType\>'.
